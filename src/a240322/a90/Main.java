...
public class Main {
  ...
  public static void main(String[] args) {
    ...
    int clothesLength = (int) (Math.random() * 30) + 1;
    String[][] clothes = new String[clothesLength][2];
    for (int i = 0; i < clothesLength; i++) {
      int clothesALength = (int) (Math.random() * 20) + 1;
      String cloth = "";
      for (int j = 0; j < clothesALength; j++) {
        String a = Character.toString((char) ((int) (Math.random() * 26) + 97));
        cloth = cloth.concat(a);
      }
      clothes[i][0] = cloth;
      clothes[i][1] = Character.toString((char) ((int) (Math.random() * 5) + 97));
    }
    ...
    // 키값으로 의상의 종류를 받음
    // 밸류로는
    // 의상의 종류를 받았을때 이미 등록되어있으면 해당 값
    // 등록되어있지 않은 의상 부위면 0을 반환
    int answer = 1;
    for (String key : clothMap.keySet()){
      answer *= (clothMap.get(key)+1);
      // 옷의 갯수를 곱하는데 안입는 경우도 있으니 +1을 해줌
    }

    answer -= 1; // 모두 안입는 경우는 빼야함
