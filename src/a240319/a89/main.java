...
public class main {
  ...
  public static void main(String[] args) {
    ...
    System.out.println("원하는 품목 : " + wantList);
    int count = 0;

    ArrayList<String> discountList = new ArrayList<>();
    for (int i = 0; i < discount.length; i++) {
      if (discountList.size() == 10) {
        discountList.remove(0);
      }
      if (discountList.size() != 10) {
        discountList.add(discount[i]);
      }
      ArrayList<String> discountListCheck = (ArrayList<String>) discountList.clone();
      Collections.sort(discountListCheck);
      System.out.println(discountListCheck);
      if (Arrays.equals(discountListCheck.toArray(), wantList.toArray())) {
        count++;
      }
